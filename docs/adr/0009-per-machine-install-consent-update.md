...
- Program Files への per-machine インストールは HKLM 管理・署名検証の慣行に沿い、
  ...
  **本決定を ISS-008 に対する恒久策と位置づける**（暫定・緩和策ではない）。
  コード署名（ISS-001）は SmartScreen 警告の抑止には有効だが、EDR の行動検知は署名の有無に
  関わらず「どこに配置され、どのように自己書き換えするか」で判定するため、署名を取得しても
  EDR 誤検知を根本から断つことはできない。配置場所と更新方式を変える本決定こそが恒久策であり、
  ISS-001 は SmartScreen 対策として別途の課題として扱う（2026-08-05 中村勝 判断）。
...
- **コード署名証明書を取得して per-user のまま継続**: 却下。EDR の行動検知は署名の有無ではなく
  「ユーザー書き込み可能領域での自己書き換え」という挙動そのものを評価するため、署名しても
  per-user のままでは誤検知リスクが残る。署名は SmartScreen 対策としては有効であり、
  ISS-001 として別途扱うが、ISS-008 の解決策にはならない。
