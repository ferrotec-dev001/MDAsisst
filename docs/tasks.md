...
- [x] **ISS-017: Program Files 配下のファイル（MDAsisst.dll等）が管理者権限でも削除できない問題の解消とADR-0013策定** (2026-08-06)
  - 原因: MSIインストーラーによる per-machine 配置時の DACL / パーミッション制御によるアクセス拒否
  - 対策: ADR-0013の策定およびインストーラー・ファイルアクセス権限の適切な管理
